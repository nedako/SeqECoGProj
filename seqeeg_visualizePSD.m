...
%             imagesc(time,frex,B)
    ...
    case 'binned_BlockGroup'
            ...
            if E.SN{1}(sn) ~= 5
                ...
                for rep = 1:2
                    ...
                    for b  = 1:length(BandInfo.bands)
                        temp = squeeze(nanmean(tempPow(:,Chan2Plot,b,:),2))';
                        T.pow = reshape(temp,numel(temp),1);
                        T.band = b*ones(size(T.pow));
                        T.NumWarpSamp = repmat([1:E.NumWarpSamp]',size(temp,2) , 1);
                        T.rep = rep*ones(size(T.pow));
                        T.sn = E.SN{1}(sn)*ones(size(T.pow));
                        M = addstruct(M , T);
                        clear T
                    end
                    D = addstruct(D , M);
        ...
        snfinal = unique(D.sn);
        for b = 1:length(BandInfo.bands)
            for sn = 1:length(snfinal)
                subplot(7,length(snfinal) , fcount)
                lineplot([D.NumWarpSamp] , D.pow , 'plotfcn' , 'nanmean',...
                    'split', D.rep , 'subset' , D.sn==snfinal(sn) & D.band==b  , 'linecolor' , col,...
                    'errorcolor' , col , 'errorbars' , repmat({'shade'} , 1 , 2) , 'shadecolor' ,col,...
                    'linewidth' , 1 , 'markertype' , repmat({'o'} , 1  , 2) , 'markerfill' , col,...
                    'markersize' , 3, 'markercolor' , col);
                hold on
                line([0 length(unique(D.NumWarpSamp))] , [0 0])
                ylabel('% change from BL' )
                xlabel('norm time' )
                title(['sn =  ' , num2str(snfinal(sn)) , ' band = ' ,num2str(b)])
            ...
            for sn = 1:length(Pall.PowTR)
                for rep = 1:2
                    ...
                    for cg = 1:length(chanGroup)
                        Tcg = Tr(:,chanGroup{cg} , :);
                        diff  = squeeze(mean(Tcg,2)); % percent change from baseline
    ...
    case 'AvgPower_SeqType_comp'
        ...
        for sn = 1:length(Pall.PowTR)
            ...
            for n = 1:length(BlockGroup)
                ...
                hold on
                plot(x{cg,n}{sn}' , p{cg,n}{sn} ,'color' , colorz{n} , 'LineWidth' , 3 );
                leg = [leg , h(n).fig];
                hold on
            end
            legend(leg , legen , 'Box' , 'off')
            title(['Percent Change in Power from Baseline in ' , sequenceType.Typetags{snid} ])
            set(gca ,  'FontSize' , 16 , 'Box' , 'off' , 'XLim' , [0 110]);%,'YLim',[-12 12] ,);
            ylabel('(%)')
            xlabel('Frequency (Hz)')
            line([0 2*length(frex)] , [0 0] , 'color' , 'k' , 'LineWidth' , 1.5)
        end
            ...
            for evnt = 1:2*(D.seqlength(1)+1)
                ...
                for bg = 1:length(BlockGroup)
                    if ~isnan(x{sn,bg,evnt}) & ~isempty(x{sn,bg,evnt})
                        ...
                        hold on
                        plot(x{sn,bg,evnt}' , p{sn,bg,evnt} , 'color' , colorz{bg} ,  'LineWidth' , 1.5)
                        leg = [leg , h(sn).fig];
                        hold on
                        grid on
                        legen = [legen , blockGroupTags{bgid}];
                        title (['Press ' ,num2str(evnt-1),' PSD for ' , seqInfo.tags{snid}])
                        set(gca , 'Box' , 'off', 'XTick' , [1:7] , 'XTickLabels' , [-40:10:20], 'YLim' , [-15 , 10] , 'FontSize' , 6)
...
%         C.ChnkPlcmnt(:,2) = 0; % make sure the first press does not enter analysis
        ...
        figure('color' , 'white')
        fcount = 1;
        Co = colorz(1:length(unique(D.seqNumb)));
        for bb = 6:-1:4
            subplot(3,1,fcount)
            lineplot([D.eventNum D.blockGroup] , D.avgPSD , 'style_thickline' , 'linecolor' , Co , 'errorcolor' , Co,...
                'subset' , D.band == bb  , 'split' , D.seqNumb , 'leg' , {'random' , 'structured'})
            title([BandInfo.bandsLab(bb) , ' Band Power'])
    ...
    case 'Aligned_singleFing_average'
...
%         C.ChnkPlcmnt(:,2) = 0; % make sure the first press does not enter analysis
        ...
        %% prepare data for testing Block Groups against eachotherfigure('color' , 'white')
        fcount = 1;
        Co = colorz(1:length(unique(D.seqNumb)));
        for bb = 6:-1:4
            subplot(3,1,fcount)
...
%             Power = (Power- repmat(BlockPow.BaseLine , 1,1,size(Power , 3)))./repmat(BlockPow.BaseLine , 1,1,size(Power , 3));
            ...
            for itr = 1:length(BlockPow.start_tr)
                line([BlockPow.start_tr(itr),BlockPow.start_tr(itr)] , [min(B),max(B)] , 'LineWidth' , 3 , 'color' , 'r');
                line([BlockPow.end_tr(itr),BlockPow.end_tr(itr)] , [min(B),max(B)] , 'LineWidth' , 1 , 'color' , 'r');
            end
