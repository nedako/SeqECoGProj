...
fastBlock = horzcat(Dout.blockGroups{1} , Dout.blockGroups{3} , Dout.blockGroups{6}, Dout.blockGroups{10},Dout.blockGroups{14});
                ...
                if ~isempty(D.TN)
                    for k = 1:length(D.TN)
                        ...
                        for jj = 1:length(events)
                            if  ~isempty(find(D.RealTime{k} == events(jj)))
                                temp(1, jj) =  find(D.RealTime{k} == events(jj));
                            else
                                temp(1, jj) = NaN;
                            end
                            if ~isempty(find(D.NormTime{k} == events(jj)))
                                Ntemp(1,jj) =  find(D.NormTime{k} == events(jj));
                            else
                                Ntemp(1,jj) = NaN;
                            end
                        end
