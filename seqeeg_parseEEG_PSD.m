    ...
    case 'TimeWarpPSD_Raw_Binned'
        ...
        for bn = 1:length(BN)
            ...
            for tn = 1 :length(D.TN)
                % prepare the individual trials to be saved as the
                % fileds of a structre to make loading easier
                eval(['D.Pow_Norm_stim{',num2str(tn),',1} = P.PSD', num2str(tn), ';']);
            end
        ...
        for bn = 1:length(BN)
            ...
            for tn = 3:length(D.TN)
                ...
                if  ~D.isError(tn) & sum(sum(~isnan(D.EventMarker{tn}))) & sum(sum(~isnan(AvgPowTR)))
                    ...
                    PSD_betweenEvnt{tn,1} = A(:,:,idx(1)-10:idx(1)-5); % prior to the first press
                    for evnt = 1:length(idx)
                        ...
                        if evnt<length(idx)
                            ...
                            PSD_betweenEvnt{tn,evnt+1} = A(:,:,pressmidPoint-3:pressmidPoint+3);
                        end
                    end
                else
